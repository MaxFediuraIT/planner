...
export class PomodoroService {
  ...
  async create(userId: string) {
    const todaySesssion = await this.getTodaySession(userId);
    
    if (todaySesssion) return todaySesssion
    const user = await this.prisma.user.findUnique({ where: { id: userId }, select: { intervalsCount: true } });    /* окрема функція userService */
